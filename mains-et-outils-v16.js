...
function saveSubmission(form,type){let d=Object.fromEntries(new FormData(form).entries());d.type=type;d.date=new Date().toISOString();let all=JSON.parse(localStorage.getItem('mainsetoutils-submissions')||'[]');all.unshift(d);localStorage.setItem('mainsetoutils-submissions',JSON.stringify(all));return d}
function renderAdmin(){let root=document.querySelector('#admin-list');if(!root)return;let all=JSON.parse(localStorage.getItem('mainsetoutils-submissions')||'[]');root.innerHTML=all.length?all.map((x,i)=>`<article class="admin-card"><span class="eyebrow">${x.type} · ${new Date(x.date).toLocaleString('fr-FR')}</span><h3>${x.nom}</h3><p>${x.email||''}</p><details><summary>Voir les informations</summary>${Object.entries(x).map(([k,v])=>`<p><strong>${k}</strong><br>${String(v).replaceAll('<','&lt;')}</p>`).join('')}</details><button class="btn remove" data-i="${i}">Supprimer</button></article>`).join(''):'<div class="empty">Aucune demande enregistrée sur cet appareil.</div>';root.querySelectorAll('.remove').forEach(b=>b.onclick=()=>{all.splice(+b.dataset.i,1);localStorage.setItem('mainsetoutils-submissions',JSON.stringify(all));renderAdmin()})}renderAdmin();
...
document.querySelectorAll('form[data-form]').forEach(form=>form.addEventListener('submit',async e=>{
  ...
  try{
    ...
    payload.set('formulaire',form.dataset.form);
    ...
    if(!response.ok||result.success!==true)throw new Error(result.message||'Envoi refusé');
    ...
    message.textContent='L’envoi n’a pas abouti. Réessayez dans quelques instants ou écrivez à contact@mainsetoutils.com.';
  }finally{
